...
void Text::TextMario()
{
	...
	life->UpdateRect(GlobalHandler::screen.left+(ListTexture::TT_MARIO_TEXT_WIDTH+20),10);
	life->Render();	
}
...
void Text::TextScore()
{
	...
	score->UpdateRect(GlobalHandler::screen.left + 50,35);
	score->Render();
}
void Text::TextCoinCount()
{
	...
	coin_count->UpdateRect(GlobalHandler::screen.left+270,10);
	coin_count->Render();
}
...
void Text::TextWorld()
{
	...
	world->UpdateRect(GlobalHandler::screen.left + 425,35);
	world->Render();

	world->RenderCross(GlobalHandler::screen.left +460, 35);

	world->setScore(GlobalHandler::mapLevel2);
	world->UpdateRect(GlobalHandler::screen.left + 465 , 35);
	world->Render();
}
void Text::TextTime()
{
	...
	time->UpdateRect(GlobalHandler::screen.left+705,35);
	time->Render();
}
