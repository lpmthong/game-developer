...
void Score::Render()
{
	...
	for(int i = arrScoreIndex - 1; i >= 0; i--)
	{
		...
		sprite->Render(GlobalHandler::_directX->_backBuffer, rectDraw.left + (arrScoreIndex - i)*15, rectDraw.top, screen.left, screen.bottom );
		//trace(L"X: %d, y:%d", rectDraw.left - screen.left + (arrScoreIndex - i)*15, rectDraw.top);
	}
}
...
void Score::RenderCross( int left, int top )
{
	...
	RECT screen = GlobalHandler::screen;
	sprite->Render(GlobalHandler::_directX->_backBuffer, left, top, screen.left, screen.bottom);	
}
